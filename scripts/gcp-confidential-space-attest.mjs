...
const region =
  args.region ?? process.env.GCP_REGION ?? process.env.TF_VAR_region ?? tfvars.region ?? 'us-central1'
const zone =
...
const project = await googleFetch(token, `https://cloudresourcemanager.googleapis.com/v1/projects/${projectId}`)
...
const predicate = {
  ...
  claims: {
    ...
    secretManagerAccessIsBoundToAttestedImageDigest: checkOk('secret.current_digest_principal_has_access'),
    runtimeServiceAccountHasNoDirectAppKmsAccess: checkOk('kms.runtime_service_account_has_no_direct_access'),
    runtimeServiceAccountHasNoDirectSecretAccess: checkOk('secret.runtime_service_account_has_no_direct_access'),
    instanceMetadataContainsNoSecretPayloads: checkOk('compute.metadata_contains_no_secret_payloads'),
...
async function collectComputeEvidence() {
  ...
  evidence.computeInstance = {
    ...
    teeEnvKeys: Object.keys(metadata).filter((key) => key.startsWith('tee-env-')).sort(),
  }

  expect('compute.instance_running', instance.status === 'RUNNING', `instance status is ${instance.status}`)
  expect(
  ...
  expect(
    ...
    instance.confidentialInstanceConfig?.confidentialInstanceType === expectedConfidentialInstanceType,
...
async function collectIamEvidence() {
  ...
  expect(
    ...
    membersFor(kmsPolicy, 'roles/cloudkms.cryptoKeyEncrypterDecrypter').includes(confidentialSpacePrincipal),
  ...
  expect(
    ...
    membersFor(secretPolicy, 'roles/secretmanager.secretAccessor').includes(confidentialSpacePrincipal),
...
async function collectSecretEvidence() {
  const secret = await googleFetch(token, `https://secretmanager.googleapis.com/v1/${secretResource}`)
...
async function collectCloudSqlEvidence() {
  ...
  expect('cloudsql.instance_runnable', instance.state === 'RUNNABLE', `Cloud SQL state is ${instance.state}`)
  expect(
...
async function collectEndpointEvidence() {
  ...
  evidence.endpoints = {
    ...
    infoHeaders: headerSubset(info.headers, ['via', 'cf-ray', 'server', 'access-control-allow-origin']),
    corsHeaders: headerSubset(cors.headers, ['access-control-allow-origin', 'access-control-allow-methods']),
  }

  expect('endpoint.dns_points_to_instance', dnsRecords.includes(publicIp), `${domainName} resolves to ${publicIp}`)
  ...
  expect(
    ...
    [200, 204].includes(cors.status) &&
      ['https://wallet.cashu.me', '*'].includes(cors.headers.get('access-control-allow-origin') ?? ''),
...
function renderMarkdown(predicate, checksum) {
  ...
  const rows = predicate.checks
    .map((check) => `| ${check.ok ? 'PASS' : 'FAIL'} | \`${check.id}\` | ${escapeTable(check.detail)} |`)
    .join('\n')
